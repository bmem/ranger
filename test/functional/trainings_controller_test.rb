...
class TrainingsControllerTest < ActionController::TestCase
  ...
  test "should create training" do
    assert_difference('Training.count') do
      post :create, :event_id => @event.id, :training => {
        :instructions => @training.instructions,
        :location => @training.location,
        :map_link => @training.map_link,
        :shift_attributes => {
          :name => @training.shift.name,
          :start_time => @training.shift.start_time,
          :end_time => @training.shift.end_time,
          :description => 'Test training shift'
        }
      }
    end
  ...
  test "should update training" do
    put :update, :id => @training, :event_id => @event.id, :training => {
      :instructions => @training.instructions, :location => @training.location,
      :map_link => @training.map_link }
    assert_redirected_to event_training_path(assigns(:event), assigns(:training))
  end
